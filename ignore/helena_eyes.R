...
#alpha, total attenuation coe¤cient
...
all.dat <- all.dat %>% 
  mutate(diam=diam(volume/1e18)*5)
...
mean.flux <- all.dat %>% 
  group_by(genus,sex) %>% 
  summarize(m_flux=mean(flux,na.rm = T)) %>% 
...
all.dat <- all.dat %>% 
  ...
  mutate(n=1:n()) %>% 
  group_by(n) %>% 
  mutate(detect=r.dist(E=m_flux,A=diam))
...
mal.male_gap_p<- mal.mod_dat3 %>% 
  ggplot(aes(male_sl,female_sl,fill=male_gap))+geom_tile(alpha=0.85)+scale_fill_viridis("male detection gap (m)",option="H")+geom_contour2(aes(z=male_gap,label = ..level..),col="black",binwidth = 20)+theme_minimal(15)+theme(legend.position = "bottom",legend.key.width= unit(0.5, 'cm'))+coord_fixed()+xlab("Male Standard Length (cm)")+ylab("Female Standard Length (cm)")
...
phot.male_gap_p<- phot.mod_dat3 %>% 
  ggplot(aes(male_sl,female_sl,fill=male_gap))+geom_tile(alpha=0.85)+scale_fill_viridis("male detection gap (m)",option="H")+geom_contour2(aes(z=male_gap,label = ..level..),col="black",binwidth = 2)+theme_minimal(15)+theme(legend.position = "bottom",legend.key.width= unit(0.5, 'cm'),legend.text = element_text(size=7))+coord_fixed()+xlab("Male Standard Length (cm)")+ylab("Female Standard Length (cm)")
...
phot.det_ratio_p<- phot.mod_dat3 %>% 
  ggplot(aes(male_sl,female_sl,fill=det_ratio))+geom_tile(alpha=0.85)+scale_fill_viridis("detectio ratio",option="H")+geom_contour2(aes(z=det_ratio,label = ..level..),col="black",binwidth = .025)+theme_minimal(9)+theme(legend.position = "bottom",axis.title.y = element_blank())+coord_fixed()
...
sex_symbols <- function(x,y,drop="Photostomias",size=10){
  d <- tibble(genus=rep(c("Malacosteus","Photostomias"),2),
              label=c(male,male,female,female),
              angele=c(-20,-20,0,0)) %>% filter(genus!=drop) %>% 
    mutate(x=x,y=y)

 a <- annotate(data=d,aes(x,y,label=label),inherit.aes=F,size=size)
  return(list(a))
}
...
all.dat.sum <-all.dat %>% 
  group_by(sex,genus) %>% 
  summarize(m=mean(detect,na.rm=T),se=se(detect))

res_dat <-  all.dat %>% 
  group_by(genus) %>% 
  na.omit() %>% 
  mutate(det_res=residuals(lm(log(detect)~sl)))

res_dat %>% 
  mutate(det_res_=exp(det_res)) %>% 
  group_by(genus,sex) %>% 
  summarise(m=mean(det_res_ %>% exp)) %>%
  group_by(genus) %>% 
  summarize(sum(m))
  
anova(lm(detect~sex,res_dat %>% filter(genus=="Malacosteus")))
anova(lm(detect~sex,res_dat %>% filter(genus=="Photostomias")))
det_dist_p <- res_dat %>% 
  group_by(genus) %>% 
  na.omit() %>% 
  mutate(det_res=residuals(lm(log(detect)~sl))) %>% 
  ggplot(aes(genus,det_res,color=sex))+geom_boxplot()+scale_color_aaas(na.translate = F)+theme_classic(18)+ylab("Residuals")+xlab("")+facet_wrap(.~genus,scales="free")+sex_symbols(x=c(0.8,0.3),y=c(-0.2,1))

det_dist_reg_p <-  all.dat %>% 
  ggplot(aes(sl,log(detect),color=sex))+geom_smooth(aes(sl,log(detect)),inherit.aes = F,color="gray80",method="lm",se=F)+
geom_point()+geom_smooth(method="lm", se=F)+scale_color_aaas(na.translate = F)+theme_classic(18)+ylab("log Det. dist. (m)")+facet_wrap(.~genus,scales="free")+xlab("Standard length (cm)")

det_p<- plot_grid(det_dist_reg_p,det_dist_p,ncol=1,axis="t",labels = c("A","B"),label_size = 20)
print(det_p)



ggsave("~/Dropbox/Documents/UGResearch/Helena/detection_plots.pdf",w=9,h=7)


po_reg_p <-  all.dat %>% 
  ggplot(aes(sl,log(po),color=sex))+
  geom_point()+geom_smooth(method="lm", se=F)+scale_color_aaas(na.translate = F)+
  theme_classic(18)+ylab(po.exp)+facet_wrap(.~genus,scales="free")+xlab("Standard length (cm)")+theme(strip.background = element_blank(),legend.position = c(0.95,0.27))

lens_reg_p <-  all.dat %>% 
  ggplot(aes(sl,log(diam*1e6),color=sex))+
  geom_point()+geom_smooth(method="lm", se=F)+scale_color_aaas(na.translate = F)+theme_classic(18)+ylab(diam.exp)+facet_wrap(.~genus,scales="free")+xlab("Standard length (cm)")+theme(strip.background = element_blank(), strip.text.x = element_blank(),legend.position = "none")

size_p<- plot_grid(po_reg_p,lens_reg_p,ncol=1,axis="t",labels = c("A","B"),label_size = 20)
print(size_p)


ggsave("~/Dropbox/Documents/UGResearch/Helena/po_plots.pdf",w=7,h=4)

  

###anova

mal.flux.aov <- anova(lm(log(flux)~log(sl)*sex,all.dat %>% filter(genus=="Malacosteus")))

phot.flux.aov <- anova(lm(log(flux)~log(sl)*sex,all.dat %>% filter(genus=="Photostomias")))

mal.diam.aov <- anova(lm(log(diam)~log(sl)*sex,all.dat %>% filter(genus=="Malacosteus")))

phot.diam.aov <- anova(lm(log(diam)~log(sl)*sex,all.dat %>% filter(genus=="Photostomias")))

mal.po.aov <- anova(lm(log(po)~log(sl)*sex,all.dat %>% filter(genus=="Malacosteus")))

phot.po.aov <- anova(lm(log(po)~log(sl)*sex,all.dat %>% filter(genus=="Photostomias")))
